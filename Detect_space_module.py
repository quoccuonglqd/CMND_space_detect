...
def convert_to_binary(image, blur_kernel_size, threshold_type,
                    threshold_argument):
    ...
    if threshold_type == 'adaptive_mean':
        return cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_MEAN_C, 
            cv2.THRESH_BINARY, threshold_argument[0], threshold_argument[1])
    elif threshold_type == 'adaptive_gaussian':
        return cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, 
            cv2.THRESH_BINARY, threshold_argument[0], threshold_argument[1])
    elif threshold_type == 'normal':
        _,thresh = cv2.threshold(gray, np.amin(gray) + threshold_argument, 255, 
            cv2.THRESH_BINARY)
        return thresh 

def check_contour(contour, image_height, image_width, y_ratio = 0.6, 
    x_ratio = 0.2):
...
def non_eroded_process(image, mode, method, x_ratio, y_ratio, contour_limit):
    ...
    contours, hierarchy = cv2.findContours(image, CONTOUR_MODE_VALUES[mode],
        CONTOUR_METHOD_VALUES[method])
    
    if (len(contours)) == 2 and check_contour(contours[0], image.shape[0],
            image.shape[1], x_ratio, y_ratio) and check_contour(contours[1],
            image.shape[0], image.shape[1], x_ratio,
            y_ratio) and min(np.max(contours[0][:,0,0]),
            np.max(contours[1][:,0,0])) < 0.75 * image.shape[1]: 
        return (image_,min(np.max(contours[0][:,0,0]),
            np.max(contours[1][:,0,0])))
    else:
        for i in range(len(contours)):
            if len(contours[i]) < contour_limit:
                cv2.drawContours(image_, contours, i, color=255,
                    thickness=cv2.FILLED)
        return (image_,None)

def eroded_detect(image, mode, method, size, number_iterations, x_ratio,
    y_ratio):
    ...
    eroded = 255 - eroded   
    contours, hierarchy = cv2.findContours(eroded, CONTOUR_MODE_VALUES[mode],
        CONTOUR_METHOD_VALUES[method])

    ls = []
    for contour in contours:
        [x,y,w,h] = cv2.boundingRect(contour)
        if check_contour(contour, image.shape[0], image.shape[1],
                x_ratio, y_ratio)==False:
            continue
        ls.append(x+w)
    ...
    if len(ls) >= 3 or len(ls) == 0 or (len(ls) == 1 and ls[0] >=
            0.75 * image.shape[1])  or min(ls) >= 0.75 * image.shape[1]:
        return None
    else:
        return min(ls)
...
def process_in_single_cfg(path, cfg, is_visualize = False):
    ...
    image_,res = non_eroded_process(thresh, cfg.mode[0], cfg.method[0],
        cfg.x_ratio, cfg.y_ratio, cfg.contour_limit)
    
    if (res!=None):
        img = image
        cv2.line(img, (res,0), (res,img.shape[0]), (255,255,0), thickness=4, 
            lineType=8, shift=0)
    ...
    res = eroded_detect(image_, cfg.mode[1], cfg.method[1],
        cfg.erode_kernel_size, cfg.number_iterations,
        cfg.x_ratio, cfg.y_ratio)
